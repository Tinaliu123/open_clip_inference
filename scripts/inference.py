...
#将所有输入图片的label存放在all_labels再去重
    ...
    prompt_num=len(template)
    for label in unique_labels:
        for text in template:
            prompt = text.format(c=label)
...
# 遍历batch进行预测
    ...
    for batch in test_data:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            zeroshot_weights = []
            for i in range(0, len(all_prompt), prompt_num):
                prompt = all_prompt[i:i+prompt_num]
                ...
                class_embedding = batch_text_features.mean(dim=0)
                class_embedding /= class_embedding.norm()
                zeroshot_weights.append(class_embedding)
            zeroshot_weights = torch.stack(zeroshot_weights, dim=1).cuda()
            logits = (100.0 * batch_image_features @ zeroshot_weights).softmax(dim=-1)
